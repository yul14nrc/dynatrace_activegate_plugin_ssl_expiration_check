...
def check(domainnames_certs, utcnow, expiry_err=default_expiry_err):
    msgs = []
    ...
    for domain, cert_chain in domainnames_certs:
        ...
        elif expires < (utcnow + datetime.timedelta(days=expiry_err)):
            msgs.append(
...
def check_domains(domains, domain_certs, utcnow, expiry_err=default_expiry_err):
    result = []
    for domainnames in domains:
        ...
        seen = set()
        earliest_expiration = None
        (dmsgs, expiration) = check(domainnames_certs, utcnow, expiry_err=expiry_err)
...
@click.option('expiry_err', '-e', '--expiry-warning', metavar='DAYS', help='Days before expiration warning.', default=default_expiry_err, type=int)
@click.argument('domain', nargs=-1)
def main(file, domain, expiry_err, verbose):
    ...
    checked_domains = check_domains(domains, domain_certs, utcnow, expiry_err=expiry_err)
